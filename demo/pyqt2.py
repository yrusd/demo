...
class MainDialog(QDialog):
    ...
    def updataData(self):
        self.label.setText(u"选择的字体：%s字体大小：%d是否特效：%s" %(self.format["fontstyle"],self.format["fontsize"],self.format["fonteffect"]))
    def FontModalDialog(self):
        dialog = FontPropertiesDlg(self)
        if dialog.exec_():
            self.format["fontstyle"] = str(dialog.FontstyleComboBox.currentText())
