...
global shenzhen,shanghai,stock,code,initcode,initInfo,initCheckBox,wantWarning,hasMessage,isRunning,isMainHide
shenzhen = ''
shanghai = ''
stock = ''
code = ''
...
def countTime():
    global shenzhen,shanghai,stock,code,initCode,initInfo,wantWarning,hasMessage
    ...
    if initcode != '':
        while initCount < len(initcode.rstrip(',').split(',')):
            ...
            #message
            if dialog.table.item(initCount,1) and dialog.table.item(initCount,10) and dialog.table.item(initCount,10).text() != '' and dialog.table.item(initCount,4) and dialog.table.item(initCount,4).text() != '' and wantWarning :#and hasMessage==False:
                curPri = dialog.table.item(initCount,4).text()
                warning = dialog.table.item(initCount,10).text().split('-')
                if len(warning) == 2:
                    if warning[0] != '' and float(curPri) >= float(warning[0]):
                        hasMessage = True
                        messageBox(str(dialog.table.item(initCount,1).text() + '↑'+curPri+'↑'+warning[0]+'↑')).show()
                    if warning[1] != '' and float(curPri) <= float(warning[1]):
                        hasMessage = True
                        messageBox(str(dialog.table.item(initCount,1).text() + '↓'+curPri+'↓'+warning[1]+'↓')).show()
                else:
                    if warning[0] != '' and float(curPri) >= float(warning[0]):
                        hasMessage = True
                        messageBox(str(dialog.table.item(initCount,1).text() + '↑'+curPri+'↑'+warning[0]+'↑')).show()
